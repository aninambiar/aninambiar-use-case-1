...
| **LLM (GitHub Models)** | `langchain-openai` | `ChatOpenAI` via GitHub Models endpoint |
...
- Python 3.11+
- An API key or token for one supported provider: Groq, Google Gemini, Anthropic, or GitHub Models
...
LLM_PROVIDER=github         # or: groq | google | anthropic
...
# GitHub Models token (if using GitHub Models)
...
Get your GitHub token at [github.com/settings/tokens](https://github.com/settings/tokens)

For GitHub Models API access, the token must be a GitHub personal access token with `models:read` permission.

If Anthropic is your selected provider and the app reports that your credit balance is too low, add Anthropic credits or switch `LLM_PROVIDER` to `groq` or `google` with a valid API key, then restart Streamlit.
...
Yes. Set `LLM_PROVIDER` to `groq`, `google`, `anthropic`, or `github` in your `.env` file. The `make_llm()` factory in `core/config.py` reads this setting and returns the appropriate LangChain chat model.
